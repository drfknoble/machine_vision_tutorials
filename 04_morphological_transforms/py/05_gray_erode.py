...
def main():
    ...
    cv.imshow("img", img)
    cv.waitKey(1)
    cv.imwrite('../images/05/01.PNG', img)
    ...
    cv.imwrite('../images/05/02.PNG', img)
    ...
    for i, s in enumerate(shapes):
        ...
        cv.waitKey(0)
        cv.imwrite(
            "data/gray_salt_eroded_{}.png".format(shapes_label[i]), eroded)
        cv.imwrite('../images/05/0{}.PNG'.format(i+3), eroded)
